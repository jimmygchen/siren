...
import { BeaconNodeSpecResults, BeaconValidatorMetricResults, SyncData } from '../../../src/types/beacon';
import {
  Diagnostics,
...
const Main:FC<MainProps> = (props) => {
  ...
  const {
    initNodeHealth, initSyncData, beaconSpec, initValMetrics,
    initValidatorCountData, initValStates, initValCaches
  } = props
  ...
  const { data: valNetworkData } = useSWRPolling<ValidatorCountResult>('/api/validator-network', {refreshInterval: 60 * 1000, fallbackData: initValidatorCountData, networkError})
  ...
  const { data: validatorMetrics } = useSWRPolling<BeaconValidatorMetricResults>('/api/validator-metrics', {refreshInterval: slotInterval, fallbackData: initValMetrics, networkError})
        ...
        <div className='w-full grid grid-cols-1 lg:block pb-12 p-4 mb-28 lg:mb-0'>
          <div className='w-full space-y-6 mb-6'>
            <div className='w-full flex flex-col items-center lg:flex-row space-y-8 lg:space-y-0 justify-between'>
              ...
              <ValidatorSummary validators={validatorStates} validatorMetrics={timedMetrics} validatorNetworkData={valNetworkData} validatorCacheData={validatorCache} />
            </div>
          ...
          <ValidatorTable validatorCacheData={validatorCache} validators={filteredValidators} isFilter view='full' />
          <ValidatorModal validator={activeValidator} validatorCacheData={validatorCache} validatorMetrics={timedMetrics} />
        </div>
      </>
    </DashboardWrapper>
  )
