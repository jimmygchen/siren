...
const useDiagnosticAlerts = () => {
  ...
  const storeAlert = useCallback((alert: AlertMessage) => {
  ...
  const updateAlert = useCallback((alert: AlertMessage) => {
    setAlerts((prevAlerts) => {
      const existsAndNotDismissed = prevAlerts.some(({ id, isDismissed }) => id === alert.id && !isDismissed);
  ...
  const dismissAlert = useCallback((alert: AlertMessage) => {
    setAlerts((prevAlerts) => {
